; Directive - ensures that only one instance of the script runs at a time, new launches will replace the old one automatically
#SingleInstance Force

; Defines a variable that stores the window title of the application
appTitle := "Warcraft I Remastered"

; EnvGet retrieves the value of the USERPROFILE environment variable - that is stored in the currentUserProfilePath variable
currentUserProfilePath := EnvGet("USERPROFILE") 

; Defines a variable to store the full address of the save game directory <=> "C:\Users\<CurrentUsername>\Saved Games\WarcraftRemastered"
saveGameDirectory := currentUserProfilePath . "\Saved Games\WarcraftRemastered" ; . = concatenation operator

; Defines a variable that will store the save counter value for in-game purposes ; global - means that the variable can be given value (and its value can be modified), as needed
global saveCounter

; Defines a variable to store the full address of the file, that will store saveCounter's value
global saveCounterFile := saveGameDirectory "\saveCounter.txt"

global oldSaveGamesCounter
global oldSaveGamesCounterFile := saveGameDirectory "\OldSaveGamesCounter.txt"


; Function that checks if the application is running and that it is active
CheckForApp() {
...
F6:: {
    ...
    ; It calls the CheckForApp() function 
    ; If it returns false, the ! (not) operator makes the condition true, so the script executes the return and stops further actions
    ; If it returns true, the script continues to execute
    if !CheckForApp()
        return
    ...
    SendInput ('QuickSave' saveCounter)
    Sleep 50
